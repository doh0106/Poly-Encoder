...
class Call_Center(): 
    def __init__(self, model, tokenizer, cand_embs_df, question_embs_df, device): 
        ...
        self.question_embs_df = question_embs_df
        self.device = device
    ...
    def cosine_score(self, query): 
        with torch.no_grad():
            ...
            question_embs = torch.stack(self.question_embs_df['embedding'].tolist(), dim=1)
            question_embs = question_embs.to('cpu').detach().numpy()[0]

            # 코사인 유사도 계산
            similarity = cosine_similarity(question_emb, question_embs)[0]
    ...
    def inference(self, query): 
        ...
        with torch.no_grad(): 
            if self.cosine_score(query)<0.93:
