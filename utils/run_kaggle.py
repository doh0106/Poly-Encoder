...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_start, int(args.num_train_epochs) + 1):
        ...
        with tqdm(total=len(train_dataloader)//args.gradient_accumulation_steps) as bar:
            ...
            if epoch%10==0: 
                model.save_pretrained(f"poly_klueroberta_model_{epoch}")
                tokenizer.save_pretrained(f"poly_klueroberta_tokenizer_{epoch}")

                model.push_to_hub(f"poly_klueroberta_model_{epoch}", use_auth_token=args.hugging_face_token)
                tokenizer.push_to_hub(f"poly_klueroberta_tokenizer_{epoch}", use_auth_token=args.hugging_face_token)
